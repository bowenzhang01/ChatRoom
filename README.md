...
ChatRoom 是一个 AI 驱动的多人角色对话模拟器。创建虚拟角色，设定世界观，让 DeepSeek API 驱动角色间的自动对话。支持导演模式（干预对话走向）和用户模式（以角色身份参与对话）。
...
- 🤖 **AI 驱动对话** — DeepSeek API 驱动多角色自动对话，支持轮流/随机/动态三种发言模式
...
ChatRoom 需要 DeepSeek API Key 才能运行：
...
1. 点击底部 **「设置」** 按钮
2. 切换到 **「API」** Tab
3. 填入 API Key 和 API Base URL
4. 点击 **「测试 API」** 验证连接
5. 成功后自动获取可用模型列表

> 也可直接编辑 `config.json`，将 `api_key` 字段替换为你的 Key。参考 [`config.example.json`](config.example.json)。
